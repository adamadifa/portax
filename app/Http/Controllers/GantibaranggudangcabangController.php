...
class GantibaranggudangcabangController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query->where('gudang_cabang_mutasi.jenis_mutasi', 'GB');
        $query->orderBy('gudang_cabang_mutasi.tanggal', 'desc');
        $query->orderBy('gudang_cabang_mutasi.created_at', 'desc');
        $gantibarang = $query->paginate(10);
        $gantibarang->appends(request()->all());
        $data['gantibarang'] = $gantibarang;
        ...
        return view('gudangcabang.gantibarang.index', $data);
    }
    ...
    public function create()
    {
        ...
        $cabang = $cbg->getCabang();
        $data['cabang'] = $cabang;
        return view('gudangcabang.gantibarang.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $lastgb = Mutasigudangcabang::select('no_mutasi')
                ->where('jenis_mutasi', 'GB')
                ->whereRaw('YEAR(tanggal)="' . $tahun . '"')
                ->whereRaw('LENGTH(no_mutasi)="10"')
                ->orderBy('no_mutasi', 'desc')
                ->first();
            $last_no_gb = $lastgb != null ? $lastgb->no_mutasi : '';
            $no_gb = buatkode($last_no_gb, $kode . substr($tahun, 2, 2), 5);
            $detail = [];
    ...
    public function edit($no_mutasi)
    {
        ...
        $data['gantibarang'] = Mutasigudangcabang::select(
            'no_mutasi',
            'tanggal',
        ...
        $cabang = $cbg->getCabang();
        $data['cabang'] = $cabang;
        return view('gudangcabang.gantibarang.edit', $data);
    }
    ...
    public function update($no_mutasi, Request $request)
    {
        ...
        try {
            ...
            $gb = Mutasigudangcabang::where('no_mutasi', $no_mutasi)->first();

            //Checking
            $cektutuplaporan_gb = cektutupLaporan($gb->tanggal, "gudangcabang");
            if ($cektutuplaporan_gb > 0) {
                return Redirect::back()->with(messageError('Periode Laporan Sudah Ditutup !'));
            }
    ...
    public function show($no_mutasi)
    {
        ...
        $data['detail'] = Detailmutasigudangcabang::select('gudang_cabang_mutasi_detail.kode_produk', 'nama_produk', 'jumlah', 'isi_pcs_dus', 'isi_pcs_pack')
            ...
            ->where('no_mutasi', $no_mutasi)
            ->get();
        return view('gudangcabang.gantibarang.show', $data);
    }
    ...
    public function destroy($no_mutasi)
    {
        ...
        DB::beginTransaction();
        try {
            $cektutuplaporan = cektutupLaporan($gb->tanggal, "gudangcabang");
            if ($cektutuplaporan > 0) {
                return Redirect::back()->with(messageError('Periode Laporan Sudah Ditutup !'));
            }
