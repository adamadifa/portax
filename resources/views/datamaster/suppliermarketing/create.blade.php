<form action="{{ route('suppliermarketing.store') }}" id="formcreateSupplier" method="POST" class="space-y-4">
    @csrf
    <!-- Header -->
    <div class="border-b border-slate-200 pb-2 mb-2 flex items-center justify-between">
        <h3 class="text-base font-bold text-slate-800">Tambah Supplier Marketing</h3>
    ...
    <div class="space-y-4 pt-1">
        ...
        <!-- Kode Supplier (Auto) -->
        <div class="relative">
            <label class="absolute -top-2 left-3 bg-white px-1 text-[10px] font-bold text-black z-10">Kode Supplier (Auto)</label>
            <input type="text" name="kode_supplier" disabled 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="relative">
                ...
                <input type="text" name="no_hp_supplier" id="no_hp_supplier" 
                    class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors placeholder-slate-400" 
                    placeholder="No. HP">
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <input type="text" name="no_rekening_supplier" id="no_rekening_supplier" 
                class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors placeholder-slate-400" 
...
<script>
    $(function() {
        // Validation Logic
        $("#formcreateSupplier").submit(function(e) {
            // Reset Error Styling
            $('#formcreateSupplier .error-message').remove();
            $('#formcreateSupplier input, #formcreateSupplier textarea').removeClass('!border-red-500 invalid-border').addClass('border-slate-300');
        ...
        $('#formcreateSupplier input, #formcreateSupplier textarea').on('input change', function() {
