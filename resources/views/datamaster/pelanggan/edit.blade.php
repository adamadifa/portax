<form action="{{ route('pelanggan.update', Crypt::encrypt($pelanggan->kode_pelanggan)) }}" id="formeditPelanggan" method="POST" enctype="multipart/form-data" class="space-y-3">
    @csrf
    @method('PUT')
    <style>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 relative">
        ...
        <div class="space-y-4 pt-1">
            ...
            <div class="relative">
                ...
                <input type="text" name="kode_pelanggan" value="{{ $pelanggan->kode_pelanggan }}" disabled 
            ...
            <div class="relative">
                ...
                <input type="text" name="nik" id="nik" value="{{ $pelanggan->nik }}"
            ...
            <div class="relative">
                ...
                <input type="text" name="no_kk" id="no_kk" value="{{ $pelanggan->no_kk }}"
                    class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors placeholder-slate-400" 
                    placeholder="No. KK">
            </div>
            ...
            <div class="relative">
                ...
                <input type="text" name="tanggal_lahir" id="tanggal_lahir" value="{{ $pelanggan->tanggal_lahir }}"
            ...
            <div class="relative">
                ...
                <textarea name="alamat_pelanggan" id="alamat_pelanggan" rows="2" 
                    ...
                    placeholder="Alamat Pelanggan">{{ $pelanggan->alamat_pelanggan }}</textarea>
            </div>
            ...
            <div class="relative">
                ...
                <textarea name="alamat_toko" id="alamat_toko" rows="2" 
                    class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors placeholder-slate-400 resize-none" 
                    placeholder="Alamat Toko">{{ $pelanggan->alamat_toko }}</textarea>
            </div>
            ...
            <div class="flex gap-2 items-start">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="no_hp_pelanggan" id="no_hp_pelanggan" value="{{ $pelanggan->no_hp_pelanggan }}"
                        class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors placeholder-slate-400 {{ $pelanggan->no_hp_pelanggan == 'NA' ? 'bg-slate-100 cursor-not-allowed' : '' }}" 
                        placeholder="No. HP" {{ $pelanggan->no_hp_pelanggan == 'NA' ? 'readonly' : '' }}>
                </div>
                <label class="flex items-center gap-2 cursor-pointer h-[40px] px-2 rounded-lg border border-transparent hover:bg-slate-50 transition-colors">
                    <input type="checkbox" class="na_nohp form-checkbox h-4 w-4 text-[#003d9e] rounded border-slate-300 focus:ring-[#003d9e]" {{ $pelanggan->no_hp_pelanggan == 'NA' ? 'checked' : '' }}>
            ...
            <div class="relative">
                ...
                <select name="kode_cabang" id="kode_cabang" class="select2Kodecabang w-full text-left" data-placeholder="Cabang">
                    ...
                    @foreach ($cabang as $c)
                        <option value="{{ $c->kode_cabang }}" {{ $pelanggan->kode_cabang == $c->kode_cabang ? 'selected' : '' }}>{{ $c->nama_cabang }}</option>
                    @endforeach
                </select>
            </div>
            @endhasanyrole
            ...
            <div class="relative">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
                    @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'] as $hari)
                        <label class="cursor-pointer group">
                            <input type="checkbox" name="hari[]" value="{{ $hari }}" class="hari-check peer hidden" {{ str_contains(strtolower($pelanggan->hari), strtolower($hari)) ? 'checked' : '' }}>
             ...
             <div class="grid grid-cols-2 gap-3">
                 ...
                 <div class="relative">
                    ...
                    <input type="text" name="limit_pelanggan" id="limit_pelanggan" value="{{ formatRupiah($pelanggan->limit_pelanggan) }}" class="money w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-right focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] placeholder-slate-400" placeholder="Limit Pelanggan">
                </div>
                 @endhasanyrole
                 ...
                 <div class="relative">
                     ...
                     <select name="ljt" id="ljt" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] text-slate-600">
                        ...
                        <option value="14" {{ $pelanggan->ljt == 14 ? 'selected' : '' }}>14 Hari</option>
                        <option value="30" {{ $pelanggan->ljt == 30 ? 'selected' : '' }}>30 Hari</option>
                        <option value="45" {{ $pelanggan->ljt == 45 ? 'selected' : '' }}>45 Hari</option>
                    </select>
                </div>
            </div>
            ...
            <div class="relative">
                ...
                <select name="status_aktif_pelanggan" id="status_aktif_pelanggan" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] text-slate-600">
                    ...
                    <option value="0" {{ $pelanggan->status_aktif_pelanggan == '0' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
        </div>
        ...
        <div class="space-y-4 pt-1">
            ...
            <!-- Form Groups Right -->
             <div class="relative">
                 ...
                 <select name="kepemilikan" id="kepemilikan" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    <option value="SW" {{ $pelanggan->kepemilikan == 'SW' ? 'selected' : '' }}>Sewa</option>
                    <option value="MS" {{ $pelanggan->kepemilikan == 'MS' ? 'selected' : '' }}>Milik Sendiri</option>
                </select>
            </div>
             ...
             <div class="relative">
                 ...
                 <select name="lama_berjualan" id="lama_berjualan" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    <option value="LU01" {{ $pelanggan->lama_berjualan == 'LU01' ? 'selected' : '' }}>< 2 Thn</option>
                    <option value="LU02" {{ $pelanggan->lama_berjualan == 'LU02' ? 'selected' : '' }}>2-5 Thn</option>
                    <option value="LU03" {{ $pelanggan->lama_berjualan == 'LU03' ? 'selected' : '' }}>> 5 Thn</option>
                </select>
            </div>
            ...
            <div class="relative">
                 ...
                 <select name="status_outlet" id="status_outlet" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    <option value="NO" {{ $pelanggan->status_outlet == 'NO' ? 'selected' : '' }}>New Outlet</option>
                    <option value="EO" {{ $pelanggan->status_outlet == 'EO' ? 'selected' : '' }}>Existing Outlet</option>
                </select>
            </div>
            ...
            <div class="relative">
                ...
                <select name="kode_klasifikasi" id="kode_klasifikasi" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    @foreach ($klasifikasi_outlet as $k)
                        <option value="{{ $k->kode_klasifikasi }}" {{ $pelanggan->kode_klasifikasi == $k->kode_klasifikasi ? 'selected' : '' }}>{{ $k->klasifikasi }}</option>
                    @endforeach
                </select>
            </div>
             ...
             <div class="relative">
                 ...
                 <select name="cara_pembayaran" id="cara_pembayaran" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    <option value="BT" {{ $pelanggan->cara_pembayaran == 'BT' ? 'selected' : '' }}>Transfer</option>
                <option value="AC" {{ $pelanggan->cara_pembayaran == 'AC' ? 'selected' : '' }}>Cash</option>
            ...
            <div class="relative">
                 ...
                 <select name="lama_langganan" id="lama_langganan" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-600">
                    ...
                    @foreach ($lama_langganan as $key => $value)
                        <option value="{{ $key }}" {{ $pelanggan->lama_langganan == $key ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="relative">
                ...
                <input type="text" name="lokasi" id="lokasi" value="{{ $pelanggan->latitude ? $pelanggan->latitude.','.$pelanggan->longitude : '' }}" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm placeholder-slate-400" placeholder="Titik Koordinat">
            </div>

            <!-- Omset -->
            <div class="relative">
                <label class="absolute -top-2 left-3 bg-white px-1 text-[10px] font-bold text-black z-10">Omset Toko</label>
                <input type="text" name="omset_toko" id="omset_toko" value="{{ formatRupiah($pelanggan->omset_toko) }}" class="money w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-right placeholder-slate-400" placeholder="Omset Toko">
            </div>

            @php
                $fotoPath = $pelanggan->foto ? '/pelanggan/' . $pelanggan->foto : '';
                $fotoExists = $pelanggan->foto && Storage::disk('public')->exists($fotoPath);
                
                $ownerPath = $pelanggan->foto_owner ? '/pelanggan/owner/' . $pelanggan->foto_owner : '';
                $ownerExists = $pelanggan->foto_owner && Storage::disk('public')->exists($ownerPath);
            @endphp
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
                ...
                <div class="relative">
                    <label for="foto" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors overflow-hidden relative">
                        <!-- Placeholder -->
                        <div id="upload-placeholder-foto" class="flex flex-col items-center justify-center text-center p-4 transition-opacity duration-300 {{ $fotoExists ? 'hidden' : '' }}">
                        ...
                        <!-- Preview Image -->
                        <div id="file-preview-foto" class="{{ $fotoExists ? 'flex' : 'hidden' }} absolute inset-0 w-full h-full items-center justify-center bg-white">
                             <img src="{{ $fotoExists ? getfotoPelanggan($pelanggan->foto) : '' }}" class="w-full h-full object-cover">
                ...
                <div class="relative">
                    <label for="foto_owner" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors overflow-hidden relative">
                        <!-- Placeholder -->
                        <div id="upload-placeholder-owner" class="flex flex-col items-center justify-center text-center p-4 transition-opacity duration-300 {{ $ownerExists ? 'hidden' : '' }}">
                             <div class="w-10 h-10 bg-blue-50 text-[#003d9e] rounded-full flex items-center justify-center mb-2 shadow-sm">
                        ...
                        <!-- Preview Image -->
                         <div id="file-preview-owner" class="{{ $ownerExists ? 'flex' : 'hidden' }} absolute inset-0 w-full h-full items-center justify-center bg-white">
                             <img src="{{ $ownerExists ? getfotoPelangganowner($pelanggan->foto_owner) : '' }}" class="w-full h-full object-cover">
                              <div class="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                                 <span class="text-white text-xs font-medium"><i class="fas fa-sync-alt mr-1"></i> Ganti Foto</span>
                             </div>
                        </div>
...
<script>
    $(function() {
        ...
        $("#formeditPelanggan .flatpickr-date").flatpickr({
            allowInput: true
        });
        
        // Select2 Initialization with form scoping
        $('#formeditPelanggan .select2Kodecabang, #formeditPelanggan .select2Kodesalesman, #formeditPelanggan .select2Kodewilayah').select2({
        ...
        // NA Checkbox Logic
        $('#formeditPelanggan .na_nohp').change(function() {
             const hpInput = $("#formeditPelanggan #no_hp_pelanggan");
            if (this.checked) {
        ...
        $('#formeditPelanggan .hari-check').on('change', function() {
            var checkedCount = $('#formeditPelanggan .hari-check:checked').length;
             if (checkedCount > 2) {
        ...
        function getsalesmanbyCabang() {
            var kode_cabang = $("#formeditPelanggan #kode_cabang").val();
            var kode_salesman = "{{ $pelanggan->kode_salesman }}";
            $.ajax({
                type: 'POST', url: '/salesman/getsalesmanbycabang',
                data: { _token: "{{ csrf_token() }}", kode_cabang: kode_cabang, kode_salesman: kode_salesman },
                success: function(respond) { $("#formeditPelanggan #kode_salesman").html(respond); }
            });
        }
        ...
        function getwilayahbyCabang() {
            ...
            $.ajax({
                ...
                data: { _token: "{{ csrf_token() }}", kode_cabang: kode_cabang, kode_wilayah: kode_wilayah },
                success: function(respond) { $("#formeditPelanggan #kode_wilayah").html(respond); }
            });
        }
        
         // Initial Load triggers
        if($("#formeditPelanggan #kode_cabang").val() != "") {
             getsalesmanbyCabang();
             getwilayahbyCabang();
        }

        $("#formeditPelanggan #kode_cabang").change(function() {
            getsalesmanbyCabang();
            getwilayahbyCabang();
        });
        ...
        handleFilePreview("#formeditPelanggan #foto", "#upload-placeholder-foto", "#file-preview-foto");
        handleFilePreview("#formeditPelanggan #foto_owner", "#upload-placeholder-owner", "#file-preview-owner");

        // Validation
        $("#formeditPelanggan").submit(function(e) {
            // Remove error styling
            $('#formeditPelanggan .error-message').remove();
            $('#formeditPelanggan input, #formeditPelanggan select, #formeditPelanggan textarea').removeClass('!border-red-500 invalid-border').addClass('border-slate-300');
        ...
        // Remove error on input logic
         $('#formeditPelanggan input, #formeditPelanggan select, #formeditPelanggan textarea').on('input change', function() {
        ...
        $('#formeditPelanggan .select2Kodecabang, #formeditPelanggan .select2Kodesalesman').on('select2:select', function (e) {
             $(this).next('.select2-container').find('.select2-selection').removeClass('!border-red-500');
             $(this).closest('.relative').next('.error-message').remove();
        });
    });
