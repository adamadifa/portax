<form action="{{ route('laporankeuangan.cetakpinjaman') }}" id="formPinjaman" target="_blank" method="POST" class="space-y-3">
    @csrf
    <div class="grid grid-cols-1 md:grid-cols-12 gap-x-4 gap-y-1">
        @hasanyrole($roles_show_cabang_pjp)
            <div class="md:col-span-12 relative">
                ...
                <select name="kode_cabang_pinjaman" id="kode_cabang_pinjaman"
                    class="w-full pl-10 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-4 focus:ring-[#003d9e]/5 transition-all appearance-none select2Kodecabangpinjaman font-medium text-slate-700">
            ...
            <div class="md:col-span-12 relative">
                ...
                <select name="kode_dept_pinjaman" id="kode_dept_pinjaman"
                    class="w-full pl-10 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-4 focus:ring-[#003d9e]/5 transition-all appearance-none select2Kodedeptpinjaman font-medium text-slate-700">
        ...
        <div class="md:col-span-6 relative">
            ...
            <input type="text" name="dari" id="dari_pinjaman"
        ...
        <div class="md:col-span-6 relative">
            ...
            <input type="text" name="sampai" id="sampai_pinjaman"
                class="w-full pl-10 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm placeholder-slate-400 focus:outline-none focus:border-[#003d9e] focus:ring-4 focus:ring-[#003d9e]/5 transition-all font-medium text-slate-700 flatpickr-date"
...
@push('myscript')
    <script>
        $(function() {
            ...
            const select2Kodedeptpinjaman = $(".select2Kodedeptpinjaman");
            if (select2Kodedeptpinjaman.length) {
                select2Kodedeptpinjaman.each(function() {
                    var $this = $(this);
                    $this.wrap('<div class="position-relative"></div>').select2({
            ...
            formPinjaman.submit(function(e) {
                const dari = formPinjaman.find('#dari_pinjaman').val();
                const sampai = formPinjaman.find('#sampai_pinjaman').val();
                ...
                if (dari == "") {
                    Swal.fire({
                        ...
                        didClose: (e) => {
                            formPinjaman.find("#dari_pinjaman").focus();
                        },
                    });
                    return false;
                } else if (sampai == "") {
                    Swal.fire({
                        ...
                        showConfirmButton: true,
                        didClose: (e) => {
                            formPinjaman.find("#sampai_pinjaman").focus();
                        },
                    });
                    return false;
