<form action="{{ route('coa_portax.update', Crypt::encrypt($coa->kode_akun)) }}" id="formCoa" method="POST" class="space-y-4">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-4">
        ...
        <div class="relative">
            ...
            <input type="text" name="kode_akun" id="kode_akun" value="{{ $coa->kode_akun ?? '' }}" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm placeholder-slate-400 focus:outline-none transition-colors" readonly>
        </div>

        <!-- Nama Akun -->
        <div class="relative">
            <label class="absolute -top-2 left-3 bg-white px-1 text-[10px] font-bold text-black z-10">Nama Akun</label>
            <input type="text" name="nama_akun" id="nama_akun" value="{{ $coa->nama_akun ?? '' }}" class="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm placeholder-slate-400 focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e] transition-colors" placeholder="Nama Akun">
        </div>
        ...
        <div class="relative">
            ...
            <select name="sub_akun" id="sub_akun" class="w-full select2Kodeakun">
                ...
                <option value="0" @selected($coa->sub_akun === '0')>0 - TOP LEVEL</option>
