...
@section('content')
    ...
    <div class="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Chart of Account (COA)</h2>
            <p class="text-slate-500 text-sm">Manage your Chart of Accounts and configurations.</p>
        </div>
        @can('coa_portax.create')
            <button id="btnCreate" class="bg-[#003d9e] hover:bg-blue-800 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm shadow-blue-200">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-3 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('coa_portax.index') }}" class="flex flex-col md:flex-row gap-4 w-full">
                <div class="relative flex-1">
                    <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
                    <input type="text" name="nama_akun" value="{{ Request('nama_akun') }}" 
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                        ...
                        <th class="px-3 py-2">Sub Akun</th>
                        <th class="px-3 py-2">Level</th>
                        <th class="px-3 py-2 text-right">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse ($allAccounts as $d)
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="px-3 py-2 text-slate-500 text-sm">{{ $loop->iteration + $allAccounts->firstItem() - 1 }}</td>
                            <td class="px-3 py-2">
                                <span class="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-mono font-medium border border-slate-200">{{ $d->kode_akun }}</span>
                            </td>
                            <td class="px-3 py-2 text-slate-800 font-medium text-sm">{{ $d->nama_akun }}</td>
                            <td class="px-3 py-2 text-slate-600 text-sm">
                                @if($d->sub_akun != '0')
                                    <span class="text-slate-500 font-mono">{{ $d->sub_akun }}</span>
                                @else
                                    <span class="text-slate-400 italic text-xs">None (Top Level)</span>
                                @endif
                            </td>
                            <td class="px-3 py-2 text-slate-500 text-sm">
                                <span class="inline-flex items-center rounded-full bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">Level {{ $d->level }}</span>
                            </td>
                            <td class="px-3 py-2 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @can('coa_portax.edit')
                                        <button class="btnEdit w-8 h-8 rounded-lg flex items-center justify-center bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors"
                                            kode_akun="{{ Crypt::encrypt($d->kode_akun) }}" title="Edit">
                                            <i class="fas fa-pencil-alt text-xs"></i>
                                        </button>
                                    @endcan
                                    @can('coa_portax.delete')
                                        <form method="POST" name="deleteform" class="deleteform inline-block"
                                            action="{{ route('coa_portax.delete', Crypt::encrypt($d->kode_akun)) }}">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <div class="text-xs text-slate-500">
                Showing {{ $allAccounts->firstItem() }} to {{ $allAccounts->lastItem() }} of {{ $allAccounts->total() }} entries
            </div>
            <div class="flex gap-1">
                {{ $allAccounts->links('pagination::tailwind') }} 
            </div>
        </div>
    </div>
...
<script>
    $(function() {
        ...
        $("#btnCreate").click(function(e) {
            e.preventDefault();
            openModal('/coaportax/create');
        });

        $(".btnEdit").click(function(e) {
            e.preventDefault();
            var kode_akun = $(this).attr("kode_akun");
            openModal('/coaportax/' + kode_akun + '/edit');
        });
