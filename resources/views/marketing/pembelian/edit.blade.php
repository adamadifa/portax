@extends('layouts.app')
@section('titlepage', 'Edit Pembelian Marketing')
@section('content')
...
<form action="{{ route('pembelianmarketing.update', Crypt::encrypt($pembelian->no_bukti)) }}" method="POST" id="formPembelian" class="mt-4 font-public-sans">
    @csrf
    @method('PUT')
        ...
        <div class="col-span-12 lg:col-span-3 space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                <div class="space-y-4">
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg overflow-hidden bg-slate-100">
                            <span class="pl-3 text-slate-400"><i class="ti ti-barcode"></i></span>
                            <input type="text" name="no_bukti" id="no_bukti" class="w-full px-2 py-2.5 text-sm border-0 bg-transparent focus:ring-0 placeholder-slate-400" placeholder="No. Bukti" value="{{ $pembelian->no_bukti }}">
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg overflow-hidden focus-within:ring-1 focus-within:ring-[#003d9e] focus-within:border-[#003d9e]">
                            ...
                            <input type="text" name="tanggal" id="tanggal" class="flatpickr-date w-full px-2 py-2.5 text-sm border-0 focus:ring-0 placeholder-slate-400" placeholder="Tanggal" value="{{ $pembelian->tanggal }}">
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-center border border-slate-300 rounded-lg overflow-hidden focus-within:ring-1 focus-within:ring-[#003d9e] focus-within:border-[#003d9e] bg-slate-50 cursor-pointer hover:bg-slate-100 transition-colors">
                            ...
                            <input type="text" name="nama_supplier" id="nama_supplier" class="w-full px-2 py-2.5 text-sm border-0 focus:ring-0 bg-transparent placeholder-slate-400 cursor-pointer" placeholder="Pilih Supplier" value="{{ $pembelian->nama_supplier }}" readonly>
                        </div>
                        <input type="hidden" id="kode_supplier" name="kode_supplier" value="{{ $pembelian->kode_supplier }}">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                 ...
                 <div class="space-y-3">
                    ...
                    <div class="{{ $pembelian->jenis_transaksi == 'T' ? '' : 'hidden' }}" id="jenis_bayar_tunai">
                         <div class="relative mt-4">
                            ...
                            <select name="jenis_bayar" id="jenis_bayar" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#003d9e] focus:ring-1 focus:ring-[#003d9e]">
                                ...
                                <option value="TN" {{ $pembelian->jenis_bayar == 'TN' ? 'selected' : '' }}>CASH</option>
                                <option value="TR" {{ $pembelian->jenis_bayar == 'TR' ? 'selected' : '' }}>TRANSFER</option>
                            </select>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // --- PREPOPULATE DATA DETAIL (EDIT MODE) ---
        @if(isset($detail) && count($detail) > 0)
            let existingRows = '';
            @foreach($detail as $d)
                existingRows += `
                    <tr id="index_{{ $d->kode_produk }}" class="hover:bg-slate-50 transition-colors border-b border-slate-100">
                        <td class="px-4 py-2.5 text-slate-700 font-medium">
                            <input type="hidden" name="kode_produk[]" value="{{ $d->kode_produk }}" class="kode_produk"/>
                            <input type="hidden" name="status_promosi_produk[]" value="0"/>
                            <input type="hidden" name="kode_kategori_diskon[]" class="kode_kategori_diskon" value=""/>
                        ...
                        <td class="px-4 py-2.5 text-center">
                           <div class="flex items-center justify-center gap-2">
                                 <a href="#" key="{{ $d->kode_produk }}" class="edit flex items-center justify-center w-8 h-8 rounded-full bg-green-50 text-green-600 hover:bg-green-100 transition-colors"><i class="ti ti-edit"></i></a>
                                 <a href="#" key="{{ $d->kode_produk }}" class="delete flex items-center justify-center w-8 h-8 rounded-full bg-red-50 text-red-600 hover:bg-red-100 transition-colors"><i class="ti ti-trash"></i></a>
                           </div>
                        </td>
                    </tr>
                `;
            @endforeach
            $('#loadproduk').append(existingRows);
            loadsubtotal();
        @endif
        // --- END PREPOPULATE ---
