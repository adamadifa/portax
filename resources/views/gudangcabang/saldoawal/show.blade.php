...
<div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-[#003d9e] to-blue-700">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h3 class="text-lg font-bold text-white">Detail Saldo Awal</h3>
            <p class="text-blue-200 text-xs">{{ $saldo_awal->kode_saldo_awal }}</p>
        </div>
    </div>
...
<div class="p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 mb-6">
        ...
        <table class="w-full text-sm">
            <tbody>
                <tr class="border-b border-slate-100">
                    <td class="py-2.5 pr-4 text-slate-400 font-medium w-32">Bulan</td>
                    <td class="py-2.5 text-slate-700 font-semibold">{{ $nama_bulan[$saldo_awal->bulan] }}</td>
                </tr>
                <tr class="border-b border-slate-100">
                    <td class="py-2.5 pr-4 text-slate-400 font-medium">Tahun</td>
                    <td class="py-2.5 text-slate-700 font-semibold">{{ $saldo_awal->tahun }}</td>
                </tr>
                <tr class="border-b border-slate-100 md:border-b-0">
                    <td class="py-2.5 pr-4 text-slate-400 font-medium">Kondisi</td>
                    <td class="py-2.5">
                        @if ($saldo_awal->kondisi == 'GS')
                            <span class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full text-xs font-medium border border-emerald-100">
                                <i class="fas fa-check-circle text-[10px]"></i> GOOD STOK
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 bg-red-50 text-red-600 px-2 py-0.5 rounded-full text-xs font-medium border border-red-100">
        ...
        <table class="w-full text-sm">
            <tbody>
                <tr class="border-b border-slate-100">
                    <td class="py-2.5 pr-4 text-slate-400 font-medium w-32">Cabang</td>
                    <td class="py-2.5 text-slate-700 font-semibold">{{ textUpperCase($saldo_awal->nama_cabang) }}</td>
                </tr>
                <tr class="border-b border-slate-100 md:border-b-0">
                    <td class="py-2.5 pr-4 text-slate-400 font-medium w-32">Tanggal</td>
                    <td class="py-2.5 text-slate-700 font-semibold">{{ DateToIndo($saldo_awal->tanggal) }}</td>
                </tr>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($detail as $d)
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-4 py-2.5 text-sm text-slate-700 font-medium">{{ $d->nama_produk }}</td>
