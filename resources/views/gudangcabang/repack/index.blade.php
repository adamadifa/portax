...
@endsection
    ...
    <div class="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Repack Gudang Cabang</h2>
            <p class="text-slate-500 text-sm">Kelola data mutasi repack produk di gudang cabang.</p>
        </div>
...
@endsection
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-3 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('repackcbg.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-x-3 gap-y-1" id="formSearch">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p>Tidak ada data mutasi repack ditemukan.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="text-xs text-slate-500">
                @if($repack->total() > 0)
                    Showing {{ $repack->firstItem() }} to {{ $repack->lastItem() }} of {{ $repack->total() }} entries
                @else
                    Showing 0 entries
                @endif
            </div>
            <div>
                {{ $repack->links('vendor.pagination.tailwind') }}
            </div>
        </div>
    </div>
...
<script>
   ...
   $(function() {
      ...
      $("#btnCreate").click(function(e) {
         ...
         $("#modalContent").load(`/repackcbg/create`);
      });
      ...
      $(".btnShow").click(function(e) {
         ...
         window.openTailwindModal();
         $("#modalContent").load(`/repackcbg/${no_mutasi}/show`);
      });
      ...
      $(".btnEdit").click(function(e) {
         ...
         var no_mutasi = $(this).attr("no_mutasi");
         window.openTailwindModal();
         $("#modalContent").load(`/repackcbg/${no_mutasi}/edit`);
      });
   });
</script>
@endpush
