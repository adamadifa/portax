<form action="{{ route('slipgaji.update', Crypt::encrypt($slipgaji->kode_gaji)) }}" method="POST" id="formSlipGaji">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col">
            <div class="form-group mb-3">
                <select name="bulan" id="bulan" class="form-select">
                    ...
                    @foreach ($list_bulan as $d)
                        <option value="{{ $d['kode_bulan'] }}" {{ $slipgaji->bulan == $d['kode_bulan'] ? 'selected' : '' }}>{{ $d['nama_bulan'] }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group mb-3">
                <select name="tahun" id="tahun" class="form-select">
                    ...
                    @for ($t = $start_year; $t <= date('Y'); $t++)
                        <option value="{{ $t }}" {{ $slipgaji->tahun == $t ? 'selected' : '' }}>{{ $t }}</option>
                    @endfor
                </select>
            </div>
        </div>
