<form action="{{ route('lainnyagudangjadi.update', Crypt::encrypt($lainnya->no_mutasi)) }}" method="post"
    id="formeditLainnya">
    @csrf
    <x-input-with-icon icon="ti ti-barcode" label="Auto" name="no_mutasi" readonly="true"
        value="{{ $lainnya->no_mutasi }}" />
    <x-input-with-icon icon="ti ti-calendar" label="Tanggal" name="tanggal" datepicker="flatpickr-date"
        value="{{ $lainnya->tanggal }}" />
    <x-input-with-icon icon="ti ti-file-description" label="Keterangan" name="keterangan"
        value="{{ $lainnya->keterangan }}" />
    <div class="form-check form-check-inline mt-3">
        <input class="form-check-input" type="radio" name="in_out" {{ $lainnya->in_out == 'I' ? 'checked' : '' }}
            id="inlineRadio1" value="I">
        <label class="form-check-label" for="inlineRadio1">IN</label>
    </div>
    <div class="form-check form-check-inline">
        <input class="form-check-input" type="radio" name="in_out" {{ $lainnya->in_out == 'O' ? 'checked' : '' }}
